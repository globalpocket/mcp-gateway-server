...
def test_cli_fatal_error_exit():
    ...
    with patch("argparse.ArgumentParser.parse_args") as mock_args:
        mock_args.return_value = MagicMock(work_dir="~/.mcp-gateway", config="gateway_config.json", mcp_config="mcp_config.json")
...
def test_cli_path_resolution_relative():
    ...
    with patch("argparse.ArgumentParser.parse_args") as mock_args, \
        ...
        with patch("asyncio.run", side_effect=mock_run_close):
            ...
            # 期待されるパスの計算
            expected_work_dir = Path("~/.mcp-gateway").expanduser().resolve()
