...
MCP Gateway Server is a pure, stateless proxy and facade layer designed to completely decouple AI agents (like Cline, Brownie, etc.) from underlying infrastructure complexity. 
...
- **Control Plane (Admin Interface):** Provides a REST API for orchestration workflows to dynamically provision or tear down backend routes without restarting the gateway. The Gateway remains completely stateless, pushing all infrastructure state (like container IDs) to the routing layer (e.g., Kong).